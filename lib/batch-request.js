...
function getFinalUrl(req, r, extraQuery) {
    // Accept either uri or url (this is what request does, we just mirror)
    var finalUrl = r.url || r.uri;

    // Convert relative paths to full paths
    if (typeof finalUrl === 'string' && /^\//.test(finalUrl) === true) {
        finalUrl = req.protocol + '://' + req.get('host') + finalUrl;
    }

    if (!_.isEmpty(extraQuery)) {
        var urlObj = url.parse(finalUrl, true);
        _.defaults(urlObj.query, extraQuery);
        urlObj.search = undefined;
        finalUrl = url.format(urlObj);
    }
...
var batchRequest = function(params) {
    ...
    //  Options on handling query params
    params.forwardQuery = (typeof params.forwardQuery === 'undefined') ? [] : params.forwardQuery;
    //  Options for the request module
    params.defaultRequest = (typeof params.defaultRequest !== 'object') ? {} : params.defaultRequest;
    ...
    var batch = function(req, res, next) {
        ...
        var requestPromises = _.reduce(requests, function(promises, r, key) {
            if (params.defaultRequest) {
                r = _.extend({}, params.defaultRequest, r);
            }
            if (!r.dependency || r.dependency === 'none') {
                r.headers = r.headers || {};

                var extraQuery = _.pick(req.query, params.forwardQuery);
                r.url = getFinalUrl(req, r, extraQuery);
